...
const bodySchema = z.object({
  ...
  status: z.enum(["active", "paused", "fulfilled", "archived"]).optional(),
  /** Rename — e.g. aligning stored briefs after a canonical-name refactor. */
  name: z.string().min(1).optional(),
});

/** Dev-only: flip a brief's status and/or rename it. */
export async function POST(req: Request): Promise<Response> {
  ...
  const { briefId, status, name } = parsed.data;
  if (!status && !name) {
    return Response.json({ ok: false, error: "status or name required" }, { status: 400 });
  }
