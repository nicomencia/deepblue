...
- **Phase 1 — Scout.** 🔨 In progress. Done (2026-07-07): Wallapop adapter, job queue (lease/report), ingestion → normalize/dedup → rule-based evaluation with confidence verdicts and open-questions lists, PGlite zero-setup dev DB, first dashboard (verified live end to end: 26 real listings → 12 shortlisted / 14 dead). Remaining: AutoScout24 adapter, model dossiers (LLM + web research, reviewed), LLM verdict enrichment, daily email digest, brief management UI, scheduled sweeps.
